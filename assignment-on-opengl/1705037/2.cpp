...
struct point pos = {50, 50, 0};
struct point u = {0, 0, 1};
struct point r = {-0.707,0.707, 0};
struct point l = {-0.707, -0.707, 0};
	...
	for(i=0;i<stacks;i++)
	{
		...
		for(j=0;j<slices;j++)
		{
		    ...
		    if(j<slices/2)
                ...
                color=j;
            glColor3f((double)color/(double)slices,(double)color/(double)slices,(double)color/(double)slices);
			glBegin(GL_QUADS);{
			    //upper hemisphere
				glVertex3f(points[i][j].x,points[i][j].y,points[i][j].z);
				glVertex3f(points[i][j+1].x,points[i][j+1].y,points[i][j+1].z);
...
void drawWheel(double height, double radius)
{
    glPushMatrix();
    glRotatef(90,1,0,0);
    drawCylinder(radius,height,25,50);
    glPopMatrix();
...
void drawSS()
{
    ...
    glTranslatef(tr_x,tr_y,radius);
    glRotatef(wheel_alpha,0,0,1);
    glRotatef(wheel_theta,0,1,0);
    drawWheel(height,radius);
}
...
void keyboardListener(unsigned char key, int x,int y){
	switch(key){
        ...
        case 's':
            wheel_theta+=amount;
            tr_x += 0.01745*radius*amount*cos(wheel_alpha*pi/180);
            tr_y += 0.01745*radius*amount*sin(wheel_alpha*pi/180);
            break;
        case 'w':
            wheel_theta-=amount;
            tr_x -= 0.01745*radius*amount*cos(wheel_alpha*pi/180);
...
void specialKeyListener(int key, int x,int y){
	switch(key){
        ...
        case GLUT_KEY_LEFT:		//down arrow key
            cameraAngle-=0.03;
            break;
        case GLUT_KEY_RIGHT:		//down arrow key
            cameraAngle+=0.03;
            break;
